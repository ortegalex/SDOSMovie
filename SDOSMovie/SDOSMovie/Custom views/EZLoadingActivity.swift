...
//  HorseTracker
//
//  Created by Alex Ortega on 2/9/18.
//  Copyright © 2018 Alex Ortega. All rights reserved.
//
...
public struct EZLoadingActivity {
    ...
    public static func showWithDelay(_ text: String, disableUI: Bool, seconds: Double)  {
        show(text, disableUI: disableUI)
        delay(seconds) { () -> () in
...
private extension UIScreen {
    class var Orientation: UIInterfaceOrientation {
        get {
            return UIApplication.shared.windows.first(where: { $0.isKeyWindow })?.windowScene?.interfaceOrientation ?? UIInterfaceOrientation(rawValue: 1)!
        }
    }
